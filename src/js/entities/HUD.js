...
class AudioControl extends me.GUI_Object {
    ...
    onClick(/* event */) {
        if (this.isMute) {
            me.audio.unmuteAll();
            this.setRegion(stuff.texture.getRegion("shadedDark13.png"));
            this.isMute = false;
        } else {
            me.audio.muteAll();
            this.setRegion(stuff.texture.getRegion("shadedDark15.png"));
            this.isMute = true;
        }
        return false;
    }
...
class ScoreItem extends me.Renderable {
    ...
    update(/*dt*/) {
        ...
        if (this.score !== stuff.data.score) {
            this.score = stuff.data.score;
            return true;
        }
        return false;
    }
     ...
     */
    draw(renderer) {
        this.font.draw(renderer, stuff.data.score, this.pos.x, this.pos.y);
    }
};
